...
class ShortValidationStrategy(ValidationStrategy):
    ...
    def move_validation_to_next_stage(self, validation_control, validation_name):
        ...
        threads_to_run = {key for thread_config in thread_config_requests for key in thread_config.keys()}

        # Remove the 1 core option as it was already sent
        threads_to_run.discard("1")
        for threads_str in threads_to_run:
            threads_int = int(threads_str)
    ...
    def _get_thread_configurations(self, request) -> dict[str, dict | None]:
        ...
        for number_of_threads in threads_for_test:
            ...
            if target_output_events_thread < min_output_events and number_of_threads != 1:
                self.logger.info(
                    (
...
def get_validation_strategy(validation_name: str) -> ValidationStrategy:
    ...
    """
    strategy = _ALL_VALIDATION_STRATEGIES[ShortValidationStrategy.name()]
    try:
