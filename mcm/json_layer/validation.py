...
class ShortValidationStrategy(ValidationStrategy):
    ...
    def submit_initial_validation(self, validation_control, validation_name):
        ...
        for config in thread_config_requests.values():
            ...
            threads_to_run.update(threads_for_request)
    ...
    def _get_thread_configurations(self, request) -> dict[str, dict]:
        ...
        event_efficiency_explanation = "# Event efficiency: `efficiency - (2 * efficiency_error)`: `%g - (2 * %g)` = %g" % (eff, eff_err, event_efficiency)
        if event_efficiency < 0:
            event_efficiency = eff - eff_err
            event_efficiency_explanation = "# Event efficiency: `efficiency - efficiency_error`: `%g - %g` = %g" % (eff, eff_err, event_efficiency)
        if event_efficiency < 0:
            event_efficiency = eff / 2.
            event_efficiency_explanation = "# Event efficiency: `efficiency / 2.`: `%g / 2.` = %g" % (eff, event_efficiency)
        if event_efficiency == 0:
            event_efficiency = 1.
        ...
        explanation.append(
            "# Input events: `int(output_events / event_efficiency)`: `int(%d / %g)` = %d" % (
        ...
        time_per_event_explanation = "# Time per event (s): %g" % (time_per_event)
        if time_per_event == 0:
            time_per_event = 1.
        ...
        for number_of_threads in threads_for_test:
            ...
            if test_runtime_for_target > max_runtime:
                ...
                explanation_case.extend([
                    ...
                    "# Target input events changed to: `maximum_runtime / time_per_event * number_of_threads`: `%g / %g * %d` = %g" % (
                        max_runtime, time_per_event, number_of_threads, target_input_events_thread
                    ),
                    "# Target output events change to: `target_input_events * event_efficiency`: `%g * %g` = %g" % (
            ...
            if test_runtime_for_target < min_runtime:
                ...
                explanation_case.extend([
                    ...
                    "# Target input events changed to: `minimum_runtime / time_per_event * number_of_threads`: `%d / %g * %d` = %g" % (
                        min_runtime, time_per_event, number_of_threads, target_input_events_thread
                    ),
                    "# Target output events changed to: `target_input_events * event_efficiency`: `%g * %g` = %g" % (
                        target_input_events_thread, event_efficiency, target_output_events_thread
                    )
                ])
