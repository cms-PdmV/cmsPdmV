...
class chained_request(json_base):
    ...
    def flow_to_next_step(self, input_dataset='',  block_black_list=[],  block_white_list=[]):
        ...
        if not self.get_attribute('chain'):
            raise self.ChainedRequestCannotFlowException(self.get_attribute('_id'), 'chained_request %s has got no root' % (self.get_attribute('_id')))
        ...
        if not rdb.document_exists(current_id):
            raise self.ChainedRequestCannotFlowException(self.get_attribute('_id'),'the current request %s does not exist'%( current_id))
        ...
        if current_request.get_attribute('approval') not in allowed_request_approvals:
            raise self.NotApprovedException(current_request.get_attribute('prepid'), current_request.get_attribute('approval'), allowed_request_approvals)
        ...
        if not adb.document_exists( original_action_id):
            raise self.ChainedRequestCannotFlowException(self.get_attribute('_id'),'the chained request spawned from %s which does not exist'%( original_action_id))
        ...
        if not self.get_attribute('prepid') in original_action_item:
            raise self.ChainedRequestCannotFlowException(self.get_attribute('_id'), 'has no valid action in %s'%( original_action_id))
        
        original_action_item = original_action_item[self.get_attribute('prepid')]
        if 'flag' not in original_action_item:
            raise self.ChainedRequestCannotFlowException(self.get_attribute('_id'), 'The action %s is malformated'%( original_action_id))
        ...
        if not 'block_number' in original_action_item or not original_action_item['block_number']:
            raise self.ChainedRequestCannotFlowException(self.get_attribute('_id'), 'The action has no valid block number')

        if current_request.get_attribute('completed_events')<=0:
            raise self.ChainedRequestCannotFlowException(self.get_attribute('_id'), 'The number of events completed is negative or null')
        ...
        if not 'sequences' in mcm_f.get_attribute('request_parameters'):
            raise self.ChainedRequestCannotFlowException(self.get_attribute('_id'),'The flow %s does not contain sequences information.'%( flow_name))
        ...
        if len(next_campaign.get_attribute('sequences')) != len(mcm_f.get_attribute('request_parameters')['sequences']):
            raise self.ChainedRequestCannotFlowException(self.get_attribute('_id'),'the sequences changes in flow %s are not consistent with the next campaign %s'% ( flow_name, next_campaign_id))
        ...
        if not mcm_f.get_attribute('approval') in allowed_flow_approvals:
            if not self.get_attribute('approval') in allowed_flow_approvals:
                raise self.ChainedRequestCannotFlowException(self.get_attribute('_id'),'Neither the flow (%s) nor the chained request (%s) approvals allow for flowing'%( mcm_f.get_attribute('approval'), self.get_attribute('approval')))
        ...
        if next_step != len(self.get_attribute('chain')):
            ...
            if not rdb.document_exists(next_id):
                raise self.ChainedRequestCannotFlowException(self.get_attribute('_id'),'The next request (%s) according to the step (%s) does not exist'%( next_id, next_step))
            next_request = request(rdb.get(next_id))
            if next_request.get_attribute('status') =='new':
                #most likely a rewind + resub
                approach='patch'
            else:
                raise self.ChainedRequestCannotFlowException(self.get_attribute('_id'),'This should never happen. (%s) is next according to step (%s), but is not in new status (%s)'%(next_id,next_step, next_request.get_attribute('status')))
        else:
        ...
        if 'staged' in original_action_item or 'threshold' in original_action_item:
            if current_campaign.get_attribute('root')!=1 and next_campaign.get_attribute('root')==1:
                if 'staged' in original_action_item:
                    ...
                    next_request.set_attribute('total_events',  int(current_request.get_attribute('completed_events') * float(original_action_item['threshold']) / 100.))
        ...
        if not request_saved:
            raise self.ChainedRequestCannotFlowException(self.get_attribute('_id'), 'Could not save the new request %s'%( next_request.get_attribute('prepid')))
