...
class SaveSeen(NotificationRESTResource):
    ...
    def post(self):
        ...
        if 'seen_notifications' not in user_json:
            return {"results": False, "message": "Seen notifications not in user document"}
        user_json['seen_notifications'].append(kwargs['notification_id'])
        self.logger.info("Saved seen for notification %s" % kwargs['notification_id'])
        return {"results": users_db.save(user_json)}
