...
class package_builder:
    ...
    class DataAlreadyExistsException(Exception):
        def __init__(self, directory=None):
            ...
            if not self.directory:
                package_builder.logger.inject('Data directory already exists. Give --force option to overwrite.',
                                              level='error', handler=package_builder.hname)
            else:
                package_builder.logger.inject('Directory already exists. Give --force option to overwrite.',
                                              level='error', handler=package_builder.hname)
    ...
    class NoValidRequestsDetectedException(Exception):
        def __init__(self):
            package_builder.logger.inject('Error: There were no valid requests in provided list', level='error',
                                          handler=package_builder.hname)
    ...
    class NotInitializedException(Exception):
        def __init__(self, msg=''):
            self.msg = str(msg)
            if self.msg:
                package_builder.logger.inject('Package is not initialized. Reason: %s' % (self.msg), level='error',
                                              handler=package_builder.hname)
            else:
                package_builder.logger.inject('Package is not initialized. ', level='error',
                                              handler=package_builder.hname)
    ...
    class NoStepsDetected(Exception):
        def __init__(self, rid):
            self.rid = str(rid)
            package_builder.logger.inject(' no valid cmsDriver commands could be built for request "%s"' % (self.rid),
                                          level='error', handler=package_builder.hname)
    ...
    class NoneTypePackageNameException(Exception):
        def __init__(self):
            package_builder.logger.inject(' Package name given was NoneType', level='error',
                                          handler=package_builder.hname)
    ...
    def __init_configuration(self):
        ...
        try:
            ...
            self.logger.inject('Could not create injection file "%s". Reason: %s' % (self.__injectAndApprove, ex),
                               level='error', handler=self.hname)

        self.logger.inject('injectAndApprove.sh script recreated for further appending', level='debug',
                           handler=self.hname)
    ...
    def __update_configuration(self, configuration, line):
        ...
        try:
            ...
            self.logger.inject('Could not update configuration file "%s". Reason: %s' % (configuration, ex),
                               level='error', handler=self.hname)
    ...
    def __prepare_request(self):
        ...
        if not fullcommand:
            self.logger.inject('Could build configuration scripts. Please check the request.', level='error',
                               handler=self.hname)
            return False
        ...
        try:
            ...
            self.logger.inject('Could not access setup.sh script. IOError: %s' % (ex), level='critical',
                               handler=self.hname)
            return False
